...
-  BAS弹幕是为了实现各个平台弹幕的一致性效果，因此弹幕舞台的设定为视频的真实区域(同字幕实现)，根据视频输出的Resolution值进行比例计算

    需要注意Web播放器在普通、宽屏和小窗模式下具有固定padding值 2px 7px，在网页全屏、全屏模式没有padding值

- BAS弹幕本体的变换点为左上角，即transform-origin:top left

- BAS弹幕的舞台距离和旋转角度不再兼容Flash，旋转角度代入正值，视点与舞台横宽度的角度总为55°

    （参考Mode7弹幕，用于计算的屏幕宽度总为300，因此为固定值，计算值为(300/2/Math.tan((Math.PI/180)*(55/2)) = 288.1473083496094px，BAS的width可变，在每次舞台宽度变更时，都要进行一次视点距离的重新计算）

- BAS弹幕依旧需要计算中间态，即seek到某播放时间时，需要绘制出这个时间所有BAS弹幕应当保持的状态
